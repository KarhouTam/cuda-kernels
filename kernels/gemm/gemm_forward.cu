...
__global__ void gemm_kernel2(const float* A, const float* B, const float* C,
  ...
  for (int k = 0; k < K; k += blockSize) {
    ...
    sharedB[threadIdx.y][threadIdx.x] = (k + threadIdx.y) < K && col < N
...
__global__ void gemm_kernel3(const float* __restrict__ const A,
                             const float* __restrict__ const B,
                             const float* __restrict__ const C,
                             float* __restrict__ const D, const int M,
...
#pragma unroll
    for (int s = 0; s < stride * stride; ++s) {
      ...
      const int smemRow = threadIdx.y * stride + r,
                smemCol = threadIdx.x * stride + c;
      sharedA[smemRow][smemCol] =
          (k + threadIdx.x * stride + c) < K && row < M
