...
__global__ void online_softmax_kernel4(float* input, float* output, const int M,
                                       const int N) {
  // one block per row
...
__global__ void online_softmax_kernel5(float* __restrict__ input,
    ...
    switch (kernel) {
      ...
      case 4:
        benchmarkKernel(repeatTimes, online_softmax_kernel4, M, blockSize,
                        blockSize / 32 * 2 * sizeof(float), 0, &elapsedTime,
                        inputGPU, outputGPU, M, N);
        break;
      case 5:
