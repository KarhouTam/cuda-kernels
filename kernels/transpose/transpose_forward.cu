...
__global__ void transpose_forward_kernel1(float* input, float* output, int M,
                                          int N) {
...
__global__ void transpose_forward_kernel2(float* input, float* output, int M,
                                          int N) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void transpose_forward_kernel3(float* input, float* output, int M,
                                          int N) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  if (row < M && col < N) {
