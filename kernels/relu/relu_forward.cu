...
__global__ void relu_forward_kernel4(float* input, float* output, int M,
                                     int N) {
  using f128 = Package128<float>;
...
int main(int argc, char** argv) {
  if (argc < 2) {
    fprintf(
        stderr,
        "Usage: relu_forward <kernel> [blockSize] [benchmarkRepeatTimes]\n");
    return EXIT_FAILURE;
  }
  ...
  switch (kernel) {
    case 1:
      relu_forward_kernel1<<<M * N / blockSize, blockSize>>>(inputGPU,
                                                             outputGPU, M, N);
      break;
    case 2:
      relu_forward_kernel2<<<M * N / blockSize, blockSize>>>(inputGPU,
                                                             outputGPU, M, N);
      break;
    case 3:
      relu_forward_kernel3<<<M * N / blockSize, blockSize>>>(inputGPU,
                                                             outputGPU, M, N);
      break;
    case 4:
      relu_forward_kernel4<<<
          ceilDiv(M * N, (blockSize * Package128<float>::size)), blockSize>>>(
  ...
  if (checkResults(output, resFromGPU, M * N)) {
    switch (kernel) {
      case 1:
        benchmarkKernel(repeatTimes, relu_forward_kernel1, M * N / blockSize,
                        blockSize, 0, 0, &elapsedTime, inputGPU, outputGPU, M,
                        N);
        break;
      case 2:
        benchmarkKernel(repeatTimes, relu_forward_kernel2, M * N / blockSize,
                        blockSize, 0, 0, &elapsedTime, inputGPU, outputGPU, M,
                        N);
        break;
      case 3:
        benchmarkKernel(repeatTimes, relu_forward_kernel3, M * N / blockSize,
                        blockSize, 0, 0, &elapsedTime, inputGPU, outputGPU, M,
                        N);
        break;
      case 4:
        benchmarkKernel(repeatTimes, relu_forward_kernel4,
                        ceilDiv(M * N, (blockSize * Package128<float>::size)),
                        blockSize, 0, 0, &elapsedTime, inputGPU, outputGPU, M,
                        N);
        break;
    }
